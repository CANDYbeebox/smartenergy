...
public class ReadExcel {
    public static void main(String[] args) throws Exception {
        ...
        File file = new File("E:/ding/read.xls");
...
//                        continue;
//                    }
                    if (sheet.getCell(19, i).getContents().isEmpty() &&
                            sheet.getCell(20, i).getContents().isEmpty() &&
    ...
    public void updateExcel(Map<Key, List> map)throws Exception{
        int cnt = 0;
        FileInputStream fis=new FileInputStream("E:/ding/result.xls");
...
//        workbook.
        ...
        for (int i = 1; i < sheet.getPhysicalNumberOfRows(); i++) {
            ...
            HSSFCell cell = sheet.getRow(i).getCell(29);
            if (cell == null) {
                System.out.println(i + ",");
            }
            if (!((String) list.get(10)).isEmpty()) {
                cell.setCellValue(Double.valueOf((String) list.get(10)));
            }

            /*
            for (int j = 2; j < list.size(); j++) {
                HSSFCell cell = sheet.getRow(i).getCell(j + 17);
                if (((String) list.get(j)).isEmpty()) {
                    continue;
                }
                cell.setCellValue(Double.valueOf((String) list.get(j)));
            }
            */
        ...
        FileOutputStream fos=new FileOutputStream("E:/ding/result.xls");
        workbook.write(fos);
        fos.close();//关闭文件输出流
